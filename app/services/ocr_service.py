...
#     MAX_WORKERS = 8  # Number of parallel OCR threads (if supported)
...
# # Default instance with auto-detection (tries parallel, falls back to sequential)
# # Can be configured via OCR_USE_PARALLEL environment variable
...
#  normal 5 way pass
    ...
    def extract_text(self, image_bytes: bytes, save_text: bool = False) -> Dict[str, Any]:
        img_array = np.frombuffer(image_bytes, np.uint8)
        image = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
        ...
        ocr_tasks = self._build_ocr_tasks(gray, enhanced)

        results: List[Dict[str, Any]] = []

        for idx, (img, cfg) in enumerate(ocr_tasks, start=1):
            logger.info(f"OCR pass {idx}/{len(ocr_tasks)}")
            result = self._run_ocr(img, cfg)
            results.append(result)

            # 🚀 EARLY EXIT
            if (
                result["avg_confidence"] >= self.CONFIDENCE_THRESHOLD
                and result["word_count"] >= self.MIN_WORDS
            ):
                logger.info("Early stopping OCR due to high confidence")
                break

        best = max(results, key=self._score_result)

        if save_text and best["raw_text"]:
            best["saved_file"] = self._save_ocr_text(image_bytes, best)

        return best

    # ================= OCR PIPELINE =================

    def _build_ocr_tasks(
        self, gray: np.ndarray, enhanced: np.ndarray
    ) -> List[Tuple[np.ndarray, str]]:
        """5-pass high signal OCR strategy"""
    ...
    def _run_ocr(self, image: np.ndarray, config: str) -> Dict[str, Any]:
        try:
            lang = "eng+hin"
            try:
                data = pytesseract.image_to_data(
    ...
    def _score_result(self, r: Dict[str, Any]) -> float:
        return (
            len(r.get("raw_text", "")) * 0.4
            + r.get("avg_confidence", 0) * 100 * 0.4
            + r.get("word_count", 0) * 0.2
        )

    def _save_ocr_text(self, image_bytes: bytes, result: Dict[str, Any]) -> str:
        image_hash = hashlib.md5(image_bytes[:1024]).hexdigest()[:8]
        ts = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = self.EXTRACT_DIR / f"ocr_{ts}_{image_hash}.txt"
